...
const PreLoader = ({ onFinish }) => {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            const textSpansCount = (pre && pre.querySelectorAll) ? pre.querySelectorAll('.texts-container span:not(.box)').length : document.querySelectorAll('.texts-container span:not(.box)').length;
            const boxDuration = spanDuration + Math.max(0, (textSpansCount - 1) * spanStagger);
            ...
            tl.to(".texts-container", {
                ...
                .to('.initials', { duration: 0.4, opacity: 0, ease: 'power1.inOut' }, '<')
    ...
    return (
        ...
        >
            ...
            >
                ...
                <span className="box" style={{width: "50px", height: "50px", backgroundColor: "#000000", opacity: 1}}></span>
                <div className="sub hidden"></div>

                {/* initials that will replace the full name texts */}
                <div className="initials absolute opacity-0 flex items-center justify-center text-[28px] font-bold" style={{pointerEvents: 'none'}}>
                    JAA
                </div>

                {/* logo image, hidden initially; path: public/images/logo_transparent.png */}
                <img src="/assets/images/logo_transparent.png" alt="logo" className="loader-logo absolute opacity-0" style={{width: 80, height: 80, objectFit: 'contain', pointerEvents: 'none'}} />
            </div>
        </div>
    );
};
